  ...
  useEffect(() => {
    const fetchSede = async () => {
      try {
        ...
        // ✅ CASO ESPECIAL: Sede Central (ID 0)
        if (sedeId === 0) {
          setSede({
            sede_id: 0,
          ...
          if (publicacion) {
            setSede({
              ...
              sede_nombre: publicacion.publicaciones_titulo
                .replace('Sede Academica de ', '')
                .replace('Sede Academica ', ''),
              sede_direccion: publicacion.publicaciones_descripcion?.replace(/<[^>]*>/g, '') || 'Por definir',
              sede_telefono: '',
              sede_coordinador: publicacion.publicaciones_autor || 'Coordinación',
              ...
              {sede.sede_coordinador && (
                <div className="mb-8 p-4 rounded-xl bg-muted/50 border">
                  <div className="flex items-start gap-3">
                    <User className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
                    <div>
                ...
                {sede.sede_id === 0 && institucion && (
                  <>
                    {institucion.institucion_correo1 && (
                      <div className="flex items-start gap-3 p-4 rounded-xl bg-muted/50 border">
                        <Mail className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: primaryColor }} />
                        <div>
                          <p className="text-sm font-medium mb-1">Correo electrónico</p>
                          <a href={`mailto:${institucion.institucion_correo1}`} className="text-primary hover:underline">
                            {institucion.institucion_correo1}
                          </a>
                        </div>
                      </div>
